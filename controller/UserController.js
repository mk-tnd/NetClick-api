...
async function register(req, res, next) {
  ...
  try {
    ...
    if (email.trim() === "")
      throw new ValidateError("Email can not be blank", 400);
    ...
    if (password.trim() === "")
      throw new ValidateError("Password can not be blank", 400);
    if (!password) throw new ValidateError("Password is required", 400);
    if (!confirmPassword)
      throw new ValidateError("Comfirm password is required", 400);
