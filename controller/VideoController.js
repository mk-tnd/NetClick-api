...
exports.getAllVideo = async (req, res, next) => {
  ...
  let search;
  role === 'admin' ? search = { include: [{ model: Category, attributes: ['name'] }] } : search = { where: { status: 'Showing' }, include: [{ model: Category, attributes: ['name'] }] }
  let condition = {}
  !category ? condition = { ...search } : role === 'admin' ? condition = { ...search, where: { categoryId: category } } : condition = { where: { status: 'Showing', categoryId: category }, include: [{ model: Category, attributes: ['name'] }] }
  try {
