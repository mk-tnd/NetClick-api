...
async function createPackage(req, res, next) {
  ...
  try {
    ...
    if (name.trim() === "")
      throw new ValidateError("Package name can not be blank", 400);
    const sendData = {
      name,
      videoQuality,
      resolutions,
...
async function editPackage(req, res, next) {
  ...
  try {
    if (!beforeUpdate)
      throw new ValidateError("Can not find this package", 400);
    if (role !== "admin") throw new ValidateError("Unauthorized", 401);
...
async function getPackage(req, res, next) {
  ...
  const searchCondition = {
    include: [
      {
        model: Package,
        attributes: ["name", "videoQuality", "resolutions", "packageTime"],
      },
    ],
  };
  ...
  try {
    role === "admin"
      ? (data = await UserPackage.findAll(condition))
      : (data = await UserPackage.findOne({
          where: { userId: id },
          ...searchCondition,
        }));
  ...
  try {
    ...
    const data = await UserPackage.findAll({
      attributes: [
        "packageId",
        [
          sequelize.fn("COUNT", sequelize.col("package_id")),
          "total_sell_package",
        ],
      ],
